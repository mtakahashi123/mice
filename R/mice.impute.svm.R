...
mice.impute.svm <- function(y, ry, x, wy = NULL, type = NULL, C = 1, scaled = TRUE, kernel = "vanilladot", ...) {
  ...
  if (length(unique(doty)) < 2) {
    # If only one class exists in the bootstrap sample,
    # we cannot fit an SVM. We return the majority (only) class.
    single_val <- as.integer(as.character(doty[1]))
    draw <- rep(single_val, sum(wy))
  } else {
